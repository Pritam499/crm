...
import { useDispatch, useSelector } from "react-redux";
import {
  fetchTasksByOpportunityId,
  deleteTask,
} from "../../../store/slices/taskSlice.js";
import TaskForm from "./TaskForm.jsx";
import Button from "../../Shared/Button.jsx";

export default function TaskList({ opportunityId }) {
  const dispatch = useDispatch();
  const { items, status } = useSelector((state) => state.tasks);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingTask, setEditingTask] = useState(null);
  ...
  useEffect(() => {
    if (opportunityId) dispatch(fetchTasksByOpportunityId(opportunityId));
  ...
  const filtered = items.filter((i) => i.opportunity_id === opportunityId);
